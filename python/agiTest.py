...
def move_node(event):
    ...
    if(res.success):
        E[0] = res.x[0] * temp1 + res.x[1] * temp2 + res.x[2] * temp3 + res.x[3] * P[int(thisID)]
        E[1] = res.x[4] * temp1 + res.x[5] * temp2 + res.x[6] * temp3 + res.x[7] * P[int(thisID)]
        E[2] = res.x[8] * temp1 + res.x[9] * temp2 + res.x[10] * temp3 + res.x[11] * P[int(thisID)]
        update_points()
        for i in range(node_num):
